/*
  ID-based signature. 多个用户的情况+聚合(用for循环)
*/
...
int main(int argc, char **argv) {
  ...
  element_t Ppub, s, P, R[K], r[K], h[K], S[K], Did[K], Qid[K], T[K], t1[K], t2[K], t3[K], t4, t5, t6, t7;
  ...
  time2 = pbc_get_time();
  //t6 = t7 = 0;
  printf("aggregate VERIFY\n");
  for(i = 0; i < K; i++) {
  element_from_hash(h[i], "Message", 7);
  element_mul_zn(t3[i], Qid[i], h[i]);
  element_add(T[i], t3[i], R[i]);
  element_add(t6, t6, S[i]);
  element_add(t7, t7, T[i]);
  }
  element_pairing(t4, P, t6);
  element_pairing(t5, Ppub, t7);
